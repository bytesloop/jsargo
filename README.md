# 使用 javascript-obfuscator 混淆 index.js

- 安装 - `npm install javascript-obfuscator -g`
- 使用 - `javascript-obfuscator app.js --output index.js`
...
nodejs-argo 是一个强大的 Argo 隧道部署工具，专为 PaaS 平台和游戏玩具平台设计。它支持多种代理协议（VLESS、VMess、Trojan 等），并集成了哪吒探针功能。
...
- 本项目自 2025 年 10 月 29 日 15 时 45 分起,已更改开源协议为 GPL 3.0,并包含以下特定要求
...
- 本项目是针对 node 环境的 paas 平台和游戏玩具而生，采用 Argo 隧道部署节点，集成哪吒探针 v0 或 v1 可选。
- node 玩具平台只需上传 index.js 和 package.json 即可，paas 平台需要 docker 部署的才上传 Dockerfile。
- 不填写 ARGO_DOMAIN 和 ARGO_AUTH 两个变量即启用临时隧道，反之则使用固定隧道。
- 哪吒 v0/v1 可选,当哪吒端口为{443,8443,2096,2087,2083,2053}其中之一时，自动开启 tls。
...
- [npm 包页面](https://www.npmjs.com/package/nodejs-argo)
...
- 感谢[ZMTO](https://zmto.com/?affid=1548)提供赞助优质双 isp vps。
